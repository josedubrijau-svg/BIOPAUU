...
(function () {
  ...
  function renderTema(t, i, status) {
    return '<div class="list-item" data-topic="' + t.id + '">' +
      ...
      '<button class="chip ' + CHIP[status] + '" data-cycle="' + t.id + '" title="Cambiar estado">' + LABEL[status] + '</button>' +
      '</div>';
  }
  ...
  function pintarResumen() {
    ...
    if (el) el.textContent = gp.done + ' de ' + gp.total + ' temas completados (' + gp.pct + '%)';
